...
class DeliveryTariff(Base):
    ...
    )  # Фиксированная базовая стоимость
...
class DeliveryOrder(Base):
    ...
    tariff_id = Column(
        Integer, ForeignKey("delivery_tariffs.id"), nullable=True, index=True
    )
    ...
    status_history = relationship(
        ...
        order_by="DeliveryOrderStatusHistory.created_at",
    )
...
class DeliveryOrderStatusHistory(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    order_id = Column(
        Integer, ForeignKey("delivery_orders.id"), nullable=False, index=True
    )
...
class UserBalanceLog(Base):
    ...
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False, index=True)
    order_id = Column(
        Integer, ForeignKey("delivery_orders.id"), nullable=True, index=True
    )

    amount = Column(
        Float, nullable=False
    )  # Отрицательное — списание, положительное — возврат
    type = Column(
        String(50), nullable=False, index=True
    )  # order_payment, refund, admin_adjustment
